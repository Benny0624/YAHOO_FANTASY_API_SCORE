...
**戰況類**
- `戰報`、`戰爆` → 本週目前對戰比分（累計到目前為止，不含排名）
- `排名` → Yahoo 官方正式排名（上週結算，固定值，不含本週即時戰況）
- `即時排名`、`即時戰況` → 推算排名：本聯盟是 Head-to-Head Categories（排名看全季累計「贏過的類別總數」，不是週對戰勝負場次），把本週目前每隊贏的類別數直接累加進季賽總類別勝場、重新排序，等於「假設現在收官」的排名，並附上每隊本週目前贏了幾類（例：`[本週目前贏 7/12 類]`）；⚠️ 非 Yahoo 官方正式結果
- `冠軍`、`猛哥`、`前三` → 官方排名前三名
- `豆汁`、`墊底`、`阿嬤都比你強` → 官方排名後三名
